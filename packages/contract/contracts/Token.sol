...
contract MyToken is ERC721, Ownable, ERC2981 {
    ...
    function tokenURI(uint256 tokenID_) public view override returns (string memory) {
        ...
        return string(abi.encodePacked(_baseTokenURI, tokenID_.toString(), ".json"));
    }
    ...
    function setTokenURI(uint256 tokenID_, string calldata uri_) external onlyOwner {
        _requireMinted(tokenID_);
        _tokenURIs[tokenID_] = uri_;
    }

    function royaltyInfo(uint256 tokenID_, uint256 salePrice_) public view override returns (address, uint256) {
        _requireMinted(tokenID_);
